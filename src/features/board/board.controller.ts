...
const BoardController = {
  ...
  post: async (req: Request, res: Response) => {
    try {
      ...
      const playerId = type === "mvp" ? req.body.playerImage : undefined; //'mvp'에서 사용할 선수 id
      const playerRecord = type === "mvp" ? req.body.playerRecord : undefined; // 'mvp'에서 사용한 선수의 해당 경기 기록
  ...
  postReply: async (req: Request, res: Response) => {
    try {
      ...
      const body = req.body.body;
      const postType = req.body.postType;
      const result = await BoardService.addReply(
        postId,
        userId,
        commentId,
        body,
  ...
  toggleLike: async (req: Request, res: Response) => {
    try {
      ...
      const postType = req.body.postType; //게시글 종류

      const result = await BoardService.toggleLike(userId, postId, postType);
