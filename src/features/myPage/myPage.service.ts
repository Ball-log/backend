...
export const getMyPageService = async (userId: string) => {
    ...
    const matchDateUTC = moment.utc(result[0].match_date);
    const res_result: getMyPageResDto = {
        ...
        match_state: result[0].match_state,
        match_date: matchDateUTC.tz("Asia/Seoul").format("YYYY-MM-DD HH:mm:ss"),
        ...
        writed_date_list: result[0].writed_date_list
            ? result.map((row) => moment.utc(row.writed_date_list).tz("Asia/Seoul")
                .format("YYYY-MM-DD"))
            : null
    };
