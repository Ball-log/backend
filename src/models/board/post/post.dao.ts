...
export const postDao = {
  ...
  postBlog: async (req: blogDto, user_id: string): Promise<number> => {
    ...
    try {
      const [result] = await connection.query<ResultSetHeader>(
        postSql.postBlog,
        [req.title, req.body, req.imgUrls[0], user_id, req.match_id]
      );
      connection.release();
  ...
  postMvp: async (req: mvpDto, user_id: string): Promise<number> => {
    ...
    try {
      const [result] = await connection.query<ResultSetHeader>(
        postSql.postMvp,
        [req.playerId, req.playerRecord, req.imgUrls[0], user_id, req.match_id]
      );
      connection.release();
      return result.insertId;
    } catch (error) {
  ...
  patchBlog: async (
    ...
    try {
      const [result] = await connection.query<ResultSetHeader>(
        postSql.patchBlog,
        [req.title, req.body, req.imgUrls[0], post_id, user_id]
      );
      connection.release();
      return result.insertId;
    } catch (error) {
  ...
  patchMvp: async (
    ...
    try {
      const [result] = await connection.query<ResultSetHeader>(
        postSql.patchMvp,
        [req.playerId, req.playerRecord, req.imgUrls[0], post_id, user_id]
      );
      connection.release();
      return result.insertId;
    } catch (error) {
