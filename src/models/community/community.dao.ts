...
const CommunityDao = {
    ...
    // type이 "league"면
        ...
        const pool = getPool();
        const [ countRows ] = await pool.execute<RowDataPacket[]>(postCountQuery, postCountParams);
    ...
    inserPost: async (
        ...
        const [ result ] = await pool.query<RowDataPacket[]>(CommunitySQL.insertPost, [
            title,
            content,
            team_id,
            user_id
    ...
    insertImageIntoPost: async (url: string, article_id: string) => {
        // 확인요망
        const pool = getPool();
        await pool.query(CommunitySQL.insertImageIntoPost, [
            url,
            article_id
        ]);
    },
